...
for entry in config:
    ...
    try:
        ...
        if len(filtered) == 0:
            print(f"⚠️ No entries for stage '{stage}' in {path}")
            continue

        # Sample the required number of entries (or take all if less)
        if len(filtered) >= count:
            sampled = filtered.sample(n=count, random_state=42)
        else:
            sampled = filtered
            print(f"⚠️ Only {len(sampled)} entries found for stage '{stage}' in {path} (requested {count})")
